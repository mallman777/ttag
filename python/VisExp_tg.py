...
if len(sys.argv)>1:
  ...
  dirnum = 9
  dataPath = 'e:\\Matlab/Qantum_Enigma/run%d/'%dirnum
...
#  image = np.reshape(getCoincidences(fTimes, fChans, gate, CoincidenceWindow, heraldChan), (1,-1))
  ...
  print pixel_num
  f = sys.stdout
  fncoin = dataPath + 'SN_coin_matrix_%d.txt'%pixel_num
  fnsing = dataPath + 'SN_single_matrix_%d.txt'%pixel_num
  np.savetxt(f, single,'%4d')
  print
  np.savetxt(f, coin,'%4d')
  print fncoin
  np.savetxt(fncoin, coin, delimiter=' ', fmt='%d')
  np.savetxt(fnsing, single, delimiter=' ', fmt='%d')
  #print '%04d'%single
  #print '%04d'%coin
  #print image
  #raw_input('hit enter')
