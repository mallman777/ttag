...
while True:
    ...
    save_thread.start()
    # Check temperature
    if checkTempGood() and detectorGood():
        retake = False
        send(cnt)
    else:
        #  Need to figure out what is not good
        #  Wait for fridge
        retake = True
        if not checkTempGood():
            #  Turn off bias
  	        while not checkTempGood():
		            time.sleep(60)
        if not detectorGood():
            for i in range(8):
                sim.conn(i+1)
            ...
            for i in range(8):
                ...
                sim.write('xyz')
            CA.firstTime = True
            #  figure out which ones need to be re-set
            #  Should we send a message if it is continually bad?
            pass
    to.append(time.time())
