...
#        buf.add(3, tag + delay)
#        buf.add(8, tag)
        buf.add(random.randint(0,8), tag + delay)
        buf.add(random.randint(8,16), tag)
        tf = time.time()
...
for i in range(len(t)):
    ...
    runTimes[0,i] = tf-to
    to = time.time()
    xx = buf.coincidences(t[i], radius, delays)
    tf = time.time()
    runTimes[1,i] = tf-to
    print np.sum(np.sum(x))
    print np.sum(np.sum(xx))
...
#plt.savefig('CoinComp_RanChans.png')
plt.figure(2)
...
#plt.savefig('CoinCompRatio_RanChans.png')
plt.show()
